...
import { IconDirective, PageSectionComponent, SkeletonComponent } from '@cigar-platform/shared/ui';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, IconDirective, PageSectionComponent, SkeletonComponent],
        ...
        @if (loading()) {
          ...
          @for (i of [1, 2, 3]; track i) {
            <div class="flex flex-col items-center p-3 md:p-4 rounded-lg bg-smoke-800 border border-smoke-700">
              <ui-skeleton variant="circular" width="24px" height="24px" class="mb-2" />
              <ui-skeleton variant="text" width="48px" height="28px" class="mb-1" />
              <ui-skeleton variant="text" width="64px" height="14px" />
            </div>
          }
        } @else {
          <!-- Stat 1: Dégustations -->
